...
void main()
{
	...
	if(hasAlbedoTexture)
	{
		albedo = texture(albedoMap, fs_in.TexCoord).rgb;//pow(texture(albedoMap, fs_in.TexCoord).rgb, 2.2);
	}

	float metallic = hasMetallicTexture ? texture(metallicMap, fs_in.TexCoord).r : metallicValue;
